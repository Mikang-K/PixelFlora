...
- **서버별 테마:** * Instance A: Red/Pink 계열
    - Instance B: Blue/Cyan 계열
    - Instance C: Green/Lime 계열 (스케일 아웃 시 추가)
- **색상 혼합:** 이미 피어있는 다른 색상의 꽃을 클릭할 경우, 두 색상이 섞인 중간색 픽셀 생성.
...
- **서버 종료 (Scale-in):** 부하 감소로 서버가 종료되면 해당 색상의 꽃들이 서서히 회색(Grayscale)으로 변함.
...
| **구분** | **기술** | **역할** |
| --- | --- | --- |
| **Frontend** | React, Canvas API | 픽셀 렌더링 및 실시간 인터렉션 |
| **Backend** | Node.js (Express) | API 처리 및 서버 고유 ID/색상 반환 |
| **Real-time** | Socket.io | 여러 사용자 간의 꽃 좌표 동기화 |
| **Infrastructure** | AWS (ALB, ASG, EC2) | 로드밸런싱 및 자동 확장 환경 구축 |
...
2. **서버 식별:** 각 EC2는 시작 시 자신의 `Instance ID`를 기반으로 고유 색상(Red, Blue 등)을 결정.
...
// 로컬: 환경변수 INSTANCE_COLOR=A
// AWS: EC2 메타데이터 API (IMDSv2) → Instance ID 해시 → 색상 결정`
...
- **로드밸런서:** 라운드 로빈(Round Robin) 등의 알고리즘에 따른 트래픽 분산 이해.
- **오토스케일링:** 인프라가 수요에 따라 유동적으로 변하는 '신축성(Elasticity)' 체득.
- **상태 관리:** 분산 환경에서 세션이나 데이터를 어떻게 중앙(Redis)에서 관리하는지 실습.